...
public class RuleSetCreator {
	...
	public RuleSet createRuleSet() {
		...
		for(List<String> l: inputSpec) { 
			...
			switch(l.get(1).strip().toUpperCase()) { //For reviewing purposes: We believed that the passed argument would have the format ["x" "y" "z"]
			 										 //For this reason we compare with "/"x/"". Since we understood that the format would be [x y z] instead
			 										 //we duplicated the switch so that it accepts both.
				case "\"STARTS_WITH\"":	lastParameter = l.get(2);
									lastParameter = lastParameter.replaceAll("\"", "");
				...
				case "POSITIONS":	lastParameter = l.get(2);
						lastParameter = lastParameter.replaceAll("\"", "");
						List<String> stringList2 = new ArrayList<String>(Arrays.asList(lastParameter.split("\\s*,\\s*")));
						List<Integer> intList2 = stringList2.stream().map(Integer::parseInt).collect(Collectors.toList());
						currentRule = factory.createRuleInPosition(lineblocks, intList2);
						break;
				default: 	;
			}
			...
			String currentString = l.get(0).strip().toUpperCase();
			
			switch(currentString) { //For reviewing purposes: We believed that the passed argument would have the format ["x" "y" "z"]
									//For this reason we compare with "/"x/"". Since we understood that the format would be [x y z] instead
			 						//we duplicated the switch so that it accepts both.
